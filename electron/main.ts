...
// Packaged builds must never follow inherited ELECTRON_DEV / NODE_ENV=development.
const isDev =
  !app.isPackaged
...
async function startBackend(): Promise<void> {
  ...
  // Keep backend inside the asar so require() resolves dotenv/express/etc.
  // (asarUnpack of backend/dist breaks Node module resolution for a child process.)
  const backendEntry = path.join(__dirname, '../backend/dist/index.js');
  if (!fs.existsSync(backendEntry)) {
    throw new Error(`Backend entry not found: ${backendEntry}`);
  }

  logStartupInfo([`[startBackend] require ${backendEntry}`]);
